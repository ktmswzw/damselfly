...
/**
 * Created by xecoder on Sun Sep 20 22:32:29 GMT+08:00 2015.
 */
...
requirejs(['jquery', 'bootstrap', 'table', 'tablezn', 'tExport', 'tExportS', 'base64', 'comm', 'message'],
    function () {
        ...
        var $table = $('#tableB').bootstrapTable({
            url: WEB_GLOBAL_CTX + '/business/test/list',
            dataType: 'json',
            cache:false,
            showToggle:true,
            showExport:true,
        ...
        $('#delete').click(function () {
            ...
            $.each(objects, function () {
                $.post(WEB_GLOBAL_CTX + "/business/test/delete/" + this.testId, function (rsp) {
                    if (rsp.successful) {
                        $.scojs_message(rsp.msg, $OK);
                        flashTable('tableB', '/business/test/list');
                    } else {
        ...
        $('#add').click(function () {
            ...
            self.location = WEB_GLOBAL_CTX + "/business/test/add";
        });
        ...
        $('#modify').click(function () {
            ...
            var objects = $table.bootstrapTable('getSelections');
            $.each(objects, function () {
                self.location = WEB_GLOBAL_CTX + "/business/test/edit/"+this.testId;
            });
        });
