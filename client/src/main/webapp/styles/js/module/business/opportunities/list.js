...
requirejs(['jquery', 'bootstrap', 'table', 'tablezn', 'tExport', 'tExportS', 'base64', 'comm', 'message'],
    function () {
        ...
        var $table = $('#tableB').bootstrapTable({
            url: WEB_GLOBAL_CTX + '/business/opportunities/list',
            dataType: 'json',
            cache:false,
            showToggle:true,
            showExport:true,
        ...
        $('#delete').click(function () {
            ...
            $.each(objects, function () {
                $.post(WEB_GLOBAL_CTX + "/business/opportunities/delete/" + this.opportunitiesId, function (rsp) {
                    if (rsp.successful) {
                        $.scojs_message(rsp.msg, $OK);
                        flashTable('tableB', '/business/opportunities/list');
                    } else {
        ...
        $('#add').click(function () {
            ...
            self.location = WEB_GLOBAL_CTX + "/business/opportunities/add";
        });
        ...
        $('#modify').click(function () {
            ...
            var objects = $table.bootstrapTable('getSelections');
            $.each(objects, function () {
                self.location = WEB_GLOBAL_CTX + "/business/opportunities/edit/"+this.opportunitiesId;
            });
        });
...
function queryParamsF(params) {
    ...
    var str = "{\"" + name + "\":\"" + value + "\"}";
    var data = eval('(' + str + ')');
    params.sortName = "opportunities_id";
